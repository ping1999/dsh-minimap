...
`
  ...
  // expressed in minimap pixels (with a floor so it stays grabbable).
  function boxHeightFor(m) {
    return Math.min(m.containerH, Math.max(8, m.clientH * m.ratio))
  }
  ...
  function createMinimapInstance(env, editorEl) {
    ...
    const onMouseUp = () => {
      if (!dragging) return
      dragging = false
      doc0.removeEventListener('mousemove', onMouseMove)
      doc0.removeEventListener('mouseup', onMouseUp)
    ...
    const onMouseDown = (e) => {
      ...
      doc0.addEventListener('mousemove', onMouseMove)
      doc0.addEventListener('mouseup', onMouseUp)
      applyDrag(e, m)
    }
